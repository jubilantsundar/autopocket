...
class ProteinLigandWorkflow:
    ...
    def __init__(self,
        ...
        Args:
            ...
            n_sample: Number of random ligands to sample (default: None, uses all)
        """
        self.base_dir = Path(base_dir)
        self.protein_sequence = self._load_protein_sequence(protein_fasta)
        self.protein_fasta_path = Path(protein_fasta)
        self.ligands = self._load_ligands(ligand_csv, n_sample=n_sample)
        ...
        # MSA path for Boltz-2 (Boltz will create dummy MSA if sequences don't match)
        self.msa_path = self.boltz_dir / "examples" / "msa" / "seq1.a3m"
    ...
    def _load_ligands(self, csv_path: str, n_sample: int = None) -> List[LigandData]:
        ...
        # Random sample if n_sample specified
        if n_sample and n_sample < len(df):
            df = df.sample(n=n_sample, random_state=42)
            logger.info(f"Selected {n_sample} random ligands from {len(df)} total")
    ...
    def _run_boltz(self, ligand: LigandData, seed: int) -> Optional[DockingResult]:
        ...
        try:
            ...
            # Add MSA path
            if self.msa_path and self.msa_path.exists():
                yaml_content['sequences'][0]['protein']['msa'] = str(self.msa_path)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of random ligands to sample (default: None, uses all)"
    )
